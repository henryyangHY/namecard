...
## 17. 修訂：跨頁延續語言（2026-09-21）

§2 原本把「URL 參數」與「跨頁延續」列為非目標，§3 把「跨頁不延續」列為已接受的取捨。
實際使用後 Henry 要求改掉：在名片頁切成中文、點進故事頁卻退回英文，不直覺。

**改動**：語言用網址參數 `?lang=zh` 承載。

- 切換語言時 `history.replaceState` 更新網址，並改寫站內連結（`index.html` ↔ `story.html`）
- `<head>` 一行 inline script 在首次繪製前讀回參數並設定 `<html lang>`，所以不會閃一下英文
- 沒有參數的網址**仍然一律是英文**，§4.1「先上英文」的原則不變
- 兩頁加 `<link rel="canonical">` 指向無參數網址，避免被當成重複頁面

**連帶效果**：中文版成為可分享的連結，QR code 會編碼當下語言的網址。
仍然不使用 localStorage / sessionStorage — 語言不跨造訪記憶。
